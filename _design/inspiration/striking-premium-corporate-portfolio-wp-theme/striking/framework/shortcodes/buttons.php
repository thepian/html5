...
function theme_shortcode_button($atts, $content = null, $code) {
	...
	$content = '<'.$tag.$id.$link.$linktarget.$bgcolor.$hoverbgcolor.$hovertextcolor.' class="button '.$size.$color.$full.$class.$aligncss.'"><span'.(($textcolor!==''||$width!='')?' style="'.$textcolor.$width.'"':'').'>' . trim($content) . '</span></'.$tag.'>';
	if($align === 'center'){
