package com.modularweb.imageGalleries {
	...
	public class PieceMaker extends MovieClip {
		...
		private function loadComplete (lc:Event) {
			...
			var originBitmap:BitmapData = Bitmap(loader.content).bitmapData.clone();
			var finalBitmap:BitmapData;
			
			var widthScale:Number = imageWidth/originBitmap.width;
			var heightScale:Number = imageHeight/originBitmap.height;
			var scale:Number = 1;
			

			if(widthScale == 1 && heightScale == 1){
				finalBitmap = originBitmap.clone();
			}else{
				if(widthScale >= 1 || heightScale >= 1){
				...
				var resizeBitmap:BitmapData = new BitmapData(originBitmap.width * scale, originBitmap.height * scale, true, 0x000000);
				
				resizeBitmap.draw(originBitmap, matrix);
				
				if(resizeBitmap.width > imageWidth){
					finalBitmap = new BitmapData(imageWidth, imageHeight);
            		finalBitmap.copyPixels(resizeBitmap, new Rectangle(int((resizeBitmap.width-imageWidth)/2), 0, imageWidth, imageHeight), new Point(0, 0));
				}else if(resizeBitmap.height > imageHeight){
					finalBitmap = new BitmapData(imageWidth, imageHeight);
            		finalBitmap.copyPixels(resizeBitmap, new Rectangle(0, int((resizeBitmap.height-imageHeight)/2), imageWidth, imageHeight), new Point(0, 0));
				}else{
					finalBitmap = originBitmap.clone();
				}
			...
			for (i = 0; i < segments; i++) {
				var image:Bitmap = new Bitmap(finalBitmap.clone());
			...
			var oneI:Bitmap = new Bitmap(finalBitmap.clone());
			imageArray.push(oneI);
			var singleI:Bitmap = new Bitmap(finalBitmap.clone());
